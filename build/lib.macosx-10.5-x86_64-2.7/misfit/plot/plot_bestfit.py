...
def plot_bestfit_model(fitEmis2D, fileout=None, noTitles=False, verbose=False):    
    ...
    """
    sh_spec_trim = _np.shape(fitEmis2D.galaxy.spec2D.flux)
    sp_rat = sh_spec_trim[1]/_np.float(sh_spec_trim[0])  # width/height
    ...
    axes = []
    for i in _six.moves.xrange(n_cols):
    ...
    re_proj = _utils.y_proj_major(re_arcsec_int, fitEmis2D.galaxy.delt_PA)
    
    # if (fitEmis2D.instrument.conv_sigma is None):
    #     # Define conv_sigma if not set:
    #     conv_FWHM = _np.sqrt(fitEmis2D.instrument.PSF_FWHM**2 - fitEmis2D.instrument_img.PSF_FWHM**2)
    #     conv_sigma = conv_FWHM/(2.*_np.sqrt(2.*_np.log(2.)))
    
    # print "FIX THIS!"
    # raise ValueError
    
    conv_sigma_galfit = fitEmis2D.instrument.PSF.PSF_FWHM/(2.*_np.sqrt(2.*_np.log(2.)))
    
    # re_sigma = re_proj/(_np.sqrt(2.*_np.log(2.)))
    # re_sigma_conv = _np.sqrt(conv_sigma_galfit**2 + re_sigma**2)
    # re_proj_arcsec = re_sigma_conv*(_np.sqrt(2.*_np.log(2.)))
    
    re_sigma = re_arcsec_int/(_np.sqrt(2.*_np.log(2.)))
    re_sigma_conv = _np.sqrt(conv_sigma_galfit**2 + re_sigma**2)
    re_major_arcsec = re_sigma_conv*(_np.sqrt(2.*_np.log(2.)))
    re_proj_arcsec = _utils.y_proj_major(re_major_arcsec, fitEmis2D.galaxy.delt_PA)
    ...
    row_color = 'orange'
    ls = ':'
    n_wave = _np.shape(fitEmis2D.galaxy.spec2D.flux)[1]
    n_sp = _np.shape(fitEmis2D.galaxy.spec2D.flux)[0]
    ...
    row_color = 'yellow'
    ls = '--'
    wh_cont = _utils.wh_continuous(fitEmis2D.galaxy.spec2D.low_snr_row_inds)
    for wh in wh_cont:
        low = _np.min(wh)
        hi = _np.max(wh)
        if low != 0:
    ...
    column_color = 'yellow'
    ls = '--'
    wh_cont = _utils.wh_continuous(fitEmis2D.galaxy.spec2D.wh_sky_fit)
    for wh in wh_cont:
        low = _np.min(wh)
        hi = _np.max(wh)
        if low != 0:
