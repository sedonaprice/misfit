...
def trim_spectrum_1D(spec1D, galaxy, trim_restwave_range=None, 
    ...
    if (trim_restwave_range is None) and (trim_obswave_range is None):
        if param_restwave_filename is None:
            ...
            param_restwave_filename = _os.path.join(d, 'trim_wavelengths_1D.param')
        
        # Read in the trimming stuff for this line:
        trim_restwave_range = _io.read_wave_range(param_restwave_filename,linename=linename)
    
    if trim_obswave_range is None:
        trim_obswave_range = _np.array(trim_restwave_range)*(1.+galaxy.z).tolist()
    ...
    if spec1D is not None:
        ...
        wh = _np.where((spec1D.obswave >= trim_obswave_range[0]) & \
...
def trim_spectrum_2D_wavelength(spec2D, galaxy, 
    ...
    if (trim_restwave_range is not None):
        ...
        trim_restwave_range = (_np.array(trim_obswave_range)/(1.+galaxy.z)).tolist()
    else:
        if param_restwave_filename is None:
            d = _os.path.join(_os.path.dirname(_os.path.dirname(__file__)), 'lib')
            param_restwave_filename = _os.path.join(d, 'line_range_restwave_2D.param')
        # Read in the trimming stuff for this line:
        trim_restwave_range = _io.read_wave_range(param_restwave_filename,linename=linename)
        trim_obswave_range = (_np.array(trim_restwave_range)*(1.+galaxy.z)).tolist()
    ...
    if inplace:
        if spec2D is not None:
            ...
            wh = _np.where((spec2D.obswave >= trim_obswave_range[0]) & \
        ...
        if spec2D is not None:
            ...
            wh = _np.where((spec2D_trim.obswave >= trim_obswave_range[0]) & \
...
def trim_spectrum_2D_spatial(spec2D, galaxy, instrument):
    ...
    #
    n_spatial_orig = _np.shape(spec2D.flux)[0]   # Number of spatial pixels in the original image
    ...
    spec2D.n_spatial = _np.shape(spec2D.flux)[0]
    ...
    spec2D.m_lims = _np.array([0, _np.shape(spec2D.flux)[0]])
...
def subtract_continuum_2D(spec2D, galaxy, fill_value=-99., no_cont_subtraction=False):
    ...
    sum_over_col_arr = _np.sqrt(_np.sum((spec2D.flux_err*spec2D.spec_mask)**2, axis=0))
    spec2D.median_flux_error = _np.median(sum_over_col_arr)
    if galaxy.debug:
    ...
    if (spec2D.wave_full_range_rest is None):
        if spec2D.wave_full_range_rest_param_filename is None:
            d = _os.path.join(_os.path.dirname(_os.path.dirname(__file__)), 'lib')
            spec2D.wave_full_range_rest_param_filename = _os.path.join(d, 'full_range_restwave_2D.param')
        spec2D.wave_full_range_rest = _io.read_wave_range(spec2D.wave_full_range_rest_param_filename,
    ...
    if (spec2D.wave_line_range_rest is None):
        if spec2D.wave_line_range_rest_param_filename is None:
            d = _os.path.join(_os.path.dirname(_os.path.dirname(__file__)), 'lib')
            spec2D.wave_line_range_rest_param_filename = _os.path.join(d, 'line_range_restwave_2D.param')
        spec2D.wave_line_range_rest = _io.read_wave_range(spec2D.wave_line_range_rest_param_filename,
                        linename=spec2D.linenames_arr[0])
    ...
    if (spec2D.mask_lines_wave_range_rest is None):
        if spec2D.mask_lines_wave_range_rest_param_filename is None:
            d = _os.path.join(_os.path.dirname(_os.path.dirname(__file__)), 'lib')
            spec2D.mask_lines_wave_range_rest_param_filename = _os.path.join(d, 'mask_lines_range_restwave_2D.param')
        spec2D.mask_lines_wave_range_rest = _io.read_wave_range(spec2D.mask_lines_wave_range_rest_param_filename,
                    linename=spec2D.linenames_arr[0])
    ...
    spec2D.cont_model = _np.zeros(spec2D.flux.shape)
    ...
    wh_line_l = _np.where((spec2D.obswave >= frange[0])&(spec2D.obswave < line_mask_range[0]))[0]
    wh_line_r = _np.where((spec2D.obswave <= frange[1])&(spec2D.obswave > line_mask_range[1]))[0]
    wh_full = _np.where((spec2D.obswave >= frange[0])&(spec2D.obswave <= frange[1]))[0]
    ...
    if not no_cont_subtraction:
        ...
        xx_poly = _np.append(spec2D.obswave[wh_line_l], spec2D.obswave[wh_line_r])
        ...
        if galaxy.debug:
            ...
            # Debug:
            # line_mask_range #
            wh_line_mask_range = _np.where((spec2D.obswave >= line_mask_range[0])&(spec2D.obswave<= line_mask_range[1]))[0]
            #_plt.plot(spec2D.obswave[wh_line_mask_range], galaxy.spec1D.flux_err[wh_line_mask_range], 'go-')
            wh_not_missing = _np.where(galaxy.spec1D.flux_err > fill_value)[0]
            _plt.plot(galaxy.spec1D.obswave[wh_not_missing], galaxy.spec1D.flux_err[wh_not_missing], 'go-')
            _plt.gca().axhline(y=_np.median(galaxy.spec1D.flux_err[wh_not_missing]), ls='-', color='teal')
        ...
        sum_over_col_arr = _np.sqrt((galaxy.spec1D.flux_err*galaxy.spec1D.spec_mask)**2)
        ...
        # Original ranges:
        xx1d = xx_poly.copy()
        yy1d = _np.append(galaxy.spec1D.flux[wh_line_l], galaxy.spec1D.flux[wh_line_r])
        yy1d_err = _np.append(galaxy.spec1D.flux_err[wh_line_l], galaxy.spec1D.flux_err[wh_line_r])
        yy1d_mask = _np.append(spec_mask_tmp[wh_line_l], spec_mask_tmp[wh_line_r])
    
        result_1d = lmfit.minimize(_utils.weighted_linear_residual, params_1d, 
        ...
        if galaxy.debug:
            ...
            wh_notmask = _np.where(yy1d_mask == 1.)[0]
            ax.errorbar(xx1d[wh_notmask]/(1.+galaxy.z), yy1d[wh_notmask], yerr=yy1d_err[wh_notmask], 
                ls='-', marker='o', color='b')
            ax.errorbar(xx1d/(1.+galaxy.z), _utils.linear_fit(result_1d.params, xx1d), 
                        ls='-', color='orange')
        ...
        for i in _six.moves.xrange(_np.shape(spec2D.flux)[0]):
            params = lmfit.Parameters()
            params.add('a', value=slope, vary=False)
            params.add('b', value=result_1d.params['b'].value)
        
            yy = _np.append(spec2D.flux_incl_cont[i,wh_line_l], spec2D.flux_incl_cont[i,wh_line_r])
            yy_err = _np.append(spec2D.flux_err[i,wh_line_l], spec2D.flux_err[i,wh_line_r])
            yy_mask = _np.append(spec2D.spec_mask[i,wh_line_l], spec2D.spec_mask[i,wh_line_r])
        
            result = lmfit.minimize(_utils.weighted_linear_residual, params, 
                            args=(xx_poly, yy, yy_err, yy_mask)) 
                        
            continuum_fit = _utils.linear_fit(result.params, spec2D.wave)
            spec2D.cont_model[i,:] = continuum_fit
        
            # Subtract continuum:
            if (_np.isfinite(continuum_fit.max())):
            ...
            if galaxy.debug:
                continuum_fit_plot = _utils.linear_fit(result.params, x_fit_plot)
...
def mask_skylines_2D(flux=None, err=None, weight=None, mask=None, 
    ...
    # From error variance sum
    sum_over_col_arr = _np.sqrt(_np.sum((err*mask_sky)**2, axis=0))
    ...
    wh_nosky = _np.setdiff1d(_six.moves.xrange(_np.shape(flux)[1]), wh_sky)
...
def mask_low_snr_rows_2D(flux=None, err=None, mask=None, mask_calc=None, 
    ...
    sum_arr = _np.sum(flux*mask_calc, axis=1)
    # Variance error array:
    err_arr = _np.sqrt(_np.sum((err*mask_calc)**2, axis=1))
    
    snr_arr = _np.abs(sum_arr/err_arr)
    high_snr_inds = _np.where(snr_arr >= snr_cut)[0]
    nrows = len(snr_arr)
    
    # Get continuous
    high_snr_inds_cont = _utils.wh_continuous(high_snr_inds)
    if len(high_snr_inds_cont) > 1:
        high_snr_inds_new = []
        for wh in high_snr_inds_cont:
            if len(wh) > 2:
                high_snr_inds_new.extend(wh)
        high_snr_inds = _np.array(high_snr_inds_new)
    ...
    if len(low_snr_row_inds) > 0:
        if (low_snr_row_inds.min() < (nrows)/2.) & (high_snr_inds.min() < low_snr_row_inds.min()):
            ...
            while ii < minind:
                low_snr_row_inds = _np.append(low_snr_row_inds, ii)
                ii += 1
    ...
    if debug:
        ...
        wh_cont = _utils.wh_continuous(low_snr_row_inds)
        n_sp = _np.shape(flux)[0]
        for wh in wh_cont:
            low = _np.min(wh)
            hi = _np.max(wh)
            if low != 0:
...
def mask_edges_1D(spec=None, spec_err=None, spec_mask=None,num_edge=10):
    ...
    unmasked_inds = _np.where(spec_mask == 1)[0]
...
def spec_mask_missing(spec=None, spec_err=None, spec_weight=None, 
    ...
    if spec_mask is None:
        spec_mask = _np.ones(spec.shape, dtype=float)
    
    # ##########################################################
    # # Mask missing values: eg, where error = 0.
    if (not nonfinite_only) & (_np.abs(spec_err).max() > 0.):
        wh_missing = _np.where(spec_err == 0.)[0]
        spec_mask[wh_missing] = 0.
    # ##########################################################
    
    # ##########################################################
    # # Mask non-finite values of data, error:
    spec_mask[_np.invert(_np.isfinite(spec))] = 0.
    spec_mask[_np.invert(_np.isfinite(spec_err))] = 0.
    
    if spec_weight is not None:
        spec_mask[_np.where(spec_weight == 0.)] = 0.
    # ##########################################################
...
def spec_nan_masking(flux=None, spec_mask=None, fill_value=-99., mask_zero=False, mask_all_masked=False):
    ...
    if not mask_all_masked:
        wh_reset = _np.where((_np.invert(_np.isfinite(flux))) & (spec_mask == 0.))
        flux[wh_reset] = fill_value
    else:
        wh_reset = _np.where((spec_mask == 0.))
        flux[wh_reset] = fill_value
    
    if mask_zero:
        # Eg, set missing errors to non-zero so they don't cause NaN problems later.
        wh_reset = _np.where((flux == 0.) & (spec_mask == 0.))
        flux[wh_reset] = fill_value
...
def wh_skylines_1D(err_spec, cutoff=None, median_err=None, full=False):
    ...
    wh_sky = _np.where(err_spec >= cutoff*median_err)[0]
    wh_nosky = _np.where(err_spec < cutoff*median_err)[0]

    if full:
        return wh_nosky, wh_sky

    else:
        wh_cont = _utils.wh_continuous(wh_nosky)
        wh_cont_sky = _utils.wh_continuous(wh_sky)
...
def vert_position(flux=None, err=None, obswave=None, obswave_val=None,
    ...
    err[err == fill_value] = _np.median(err)*3.
    
    # Find the closest pixel to the linecenter:
    wh_exact = _np.where(abs(obswave-obswave_val) == abs(obswave-obswave_val).min())[0][0]
    
    
    
    # Straight forward sum over the wavelength direction:
    sp_sum_tmp = _np.sum(flux,axis=1)
    err_sum_tmp = _np.sqrt(_np.sum(err**2,axis=1))
    
    sn_orig = _np.abs(sp_sum_tmp/err_sum_tmp)
    sn_orig[~_np.isfinite(sn_orig)] = 0.
    ...
    if not refit:
        ...
        xx = _np.array(_six.moves.xrange(len(sp_arr)))
        
        if dither:
            result = lmfit.minimize(_utils.vert_triple_gaus_residual, params, 
                            args=(xx, sp_arr, err_arr))
        else:
            result = lmfit.minimize(_utils.gaus_residual, params, 
                            args=(xx, sp_arr, err_arr))
            
        
        m0 = _np.int(_np.round(result.params['mu'].value))
        m0_shift_fit = result.params['mu'].value - m0
        m0_peak_snr = snr_arr[m0]
        if dither:
            m1 = _np.int(_np.round(result.params['mu_l'].value))
            m2 = _np.int(_np.round(result.params['mu_r'].value))
        ...
        if debug:
            ...
            xx_arr_fine = _np.linspace(0,xx.max(), num=len(xx)*10, endpoint=True)
            if dither:
                tmp_fit = _utils.vert_triple_gaus_profile(result.params, xx_arr_fine)
            else:
                tmp_fit = _utils.gaus_profile(result.params, xx_arr_fine)
        ...
        if dither:
            ...
            cut = _np.int(_np.ceil(diff/2.))
        else:
            cut = _np.int(_np.ceil(1.5/spatial_pixscale/2.))
        ...
        # Check if it's too far off for the refit!
            ...
            if ( ((m_max-m_min)*spatial_pixscale > 4.) | (m_max-m_min < 5)):
                ...
                # Need to add conditional relational criteria:
                print("% misfit.galaxy_utils: 10.*sp_arr.max()=", 10.*sp_arr.max())
                try:
                ...
                m0 = _np.int(_np.round(result.params['mu'].value)) + trimval
                m0_shift_fit = result.params['mu'].value + trimval - m0
                m0_peak_snr = snr_arr[m0-trimval]
                if dither:
                    m1 = _np.int(_np.round(result.params['mu_l'].value)) + trimval
                    m2 = _np.int(_np.round(result.params['mu_r'].value)) + trimval
...
def calculate_2D_fit_weights(spec2D):
    ...
    if spec2D.weighting_type == 'up-edges':
        ...
        sum_arr = _np.sum(spec2D.flux*spec2D.mask_sky, axis=1)
        ## Variance error arr
        err_arr = _np.sqrt(_np.sum((spec2D.flux_err*spec2D.mask_sky)**2, axis=1))
        snr_arr = _np.abs(sum_arr/err_arr)
    ...
    # ## *No* weighting test:
        ...
        weights_nonnorm = _np.ones(spec2D.flux.shape[0])
    ## ++++++++++++++++++++++++++++++++++++++++++++
    ...
    ## *OPTIMAL* weighting test, esp for dispersion-only objects:
        ...
        sum_arr = _np.abs(_np.sum(spec2D.flux_incl_cont*spec2D.mask_sky, axis=1))
        # Need to fit this to a gaussian:
        err_arr = _np.abs(_np.sum(spec2D.flux_err*spec2D.mask_sky, axis=1))
        xx_arr = _np.linspace(0,len(sum_arr), num=len(sum_arr), endpoint=False)
        ...
        gauss_fit = _utils.gaus_profile(result.params, xx_arr)
    ...
    spec2D.fitting_weight_matrix = _np.array([spec2D.fitting_weight_profile for i in _six.moves.xrange(spec2D.flux.shape[1])]).T
...
def calculate_slit_offset_angles(galaxy):
    ...
    if not galaxy.generate_model:
        ...
        delt_PA = _np.arctan(_np.tan(delt_PA*_np.pi/180.))*180./_np.pi
    
        galaxy.delt_PA = delt_PA
        
    galaxy.delt_PA_abs = _np.abs(galaxy.delt_PA)
...
def get_m0_lam0_pos_2D(spec2D, galaxy):
    ...
    #######
    # m0_shift, lam0 = _utils.m0_shift_lam0_gausfits(self.spec2D)
    ...
    yy_arr = _np.linspace(spec2D.m_lims[0], spec2D.m_lims[1], 
    ...
    lam_prof = _np.sum(spec, axis=0)
    lam_prof_err = _np.sqrt(_np.sum(err**2, axis=0))
    ...
    if (len(wh_not_mask_y) > 0) & (len(wh_not_mask_lam) > 0):
        ...
        try:
            ...
            if ((result.params['mu'].value >= yy_arr.min()) & \
                ...
                if _np.abs(spec2D.ypos -result.params['mu'].value ) <= 2.:
                     m0_shift = result.params['mu'].value - spec2D.m0
                elif (_np.abs(spec2D.m0 -result.params['mu'].value ) <= 2.) & \
                    (_np.abs(spec2D.m0_shift_fit) <= 1.) & (spec2D.m0_peak_snr >= 3.):
        ...
        try:
            ...
            if ((result.params['mu'].value >= lam_arr.min() + wave_range/4.) & \
                ...
                if _np.abs(lam0_init -result.params['mu'].value ) <= wave_range/4.:
...
def fit_emission_y_profile(spec2D, gal, inst, filename_plot=None, plot=False, num_MC=500):
    ...
    if (len(wh_not_mask_y) > 0):
        ...
        try:
            ...
            value_matrix = _np.zeros((num_MC, len(values_true)))
