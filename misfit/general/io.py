...
def read_restwave(param_filename, linename=None):
    ...
    wh = np.where(data['linename'] == linename)[0]
    if len(wh) == 0:
...
def read_wave_range(param_filename,linename=None):
    ...
    trim_restwave_range = np.array([data['wave_l'][wh], data['wave_u'][wh]])
