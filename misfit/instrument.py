...
class PSFMoffat(PSFBase):
    ...
    def generate_conv_stamp(self, x, y, PSF_FWHM=None):
        ...
        if PSF_FWHM is None:
            PSF_FWHM = self.PSF_FWHM
        beta = self.PSF_beta
        alpha = PSF_FWHM/(2. * np.sqrt(np.power(2., 1./beta) -1))
...
def PSF_stamp_gauss(x, y, PSF_FWHM=None, yspace_dither_arc=None):
    ...
    if yspace_dither_arc is not None:
        conv_stamp_negl = np.exp(- (xarr_flat**2 + (yarr_flat+yspace_dither_arc)**2)/(2.*(PSF_sigma**2)))
        conv_stamp_negu = np.exp(- (xarr_flat**2 + (yarr_flat-yspace_dither_arc)**2)/(2.*(PSF_sigma**2)))
...
def PSF_stamp_moffat(x, y, alpha=None, beta=None, yspace_dither_arc=None):
    ...
    if yspace_dither_arc is not None:
        conv_stamp_negl = (beta-1)/(np.pi * alpha**2) * \
                np.power((1.+(xarr_flat **2 + (yarr_flat+yspace_dither_arc)**2)/(alpha**2)), -beta)
        conv_stamp_negu = (beta-1)/(np.pi * alpha**2) * \
                np.power((1.+(xarr_flat **2 + (yarr_flat-yspace_dither_arc)**2)/(alpha**2)), -beta)
        conv_stamp -= 0.5*(conv_stamp_negl + conv_stamp_negu)
