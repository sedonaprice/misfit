...
class AperModel1DDispBase(AperModel1DBase):
    ...
    def setup_model_grid(self,re_arcsec = None, q=None, delt_PA=None):
        ...
        if self.extraction_width is None:
            re_arcsec_conv = 0.5*np.sqrt((2.*re_arcsec)**2 + self.instrument.PSF.PSF_FWHM**2)
            minor_conv_arcsec = 0.5*np.sqrt((2.*q*re_arcsec)**2 + \
                        self.instrument.PSF.PSF_FWHM**2)
            q_conv = minor_conv_arcsec/re_arcsec_conv

            x_proj_conv = utils.x_proj_major(re_arcsec_conv, delt_PA, q_conv)
            y_proj_conv = utils.y_proj_major(re_arcsec_conv, delt_PA)
            # HWHM
        ...
        self.x_arr = np.linspace((-2.*self.nPixels-1.)*self.delt_x,
                    (2.*self.nPixels+1.)*self.delt_x, num=self.nX)
        self.y_arr = np.linspace((-2.*self.nPixels-1.)*self.delt_y,
...
class AperModel1DDisp(AperModel1DDispBase):
    ...
    def setup_calcs(self):
        self.galaxy.re_arcsec_circ = self.galaxy.re_arcsec* np.sqrt(self.galaxy.q)
        self.galaxy.re_mass_arcsec_circ = self.galaxy.re_mass_arcsec* np.sqrt(self.galaxy.q_mass)
    ...
    def calc_aper_correction(self):
        self.sigma_ap = kfuncs.sigma_aper_dispersion(aperModel1DDisp = self,
                        re_arcsec=self.galaxy.re_arcsec_circ,
                        re_mass_arcsec=self.galaxy.re_mass_arcsec_circ)

        self.sigma_e = kfuncs.sigma_e_dispersion(aperModel1DDisp = self,
                        re_arcsec=self.galaxy.re_arcsec_circ,
                        re_mass_arcsec=self.galaxy.re_mass_arcsec_circ,
...
class AperModel1DDispMisalign(AperModel1DDispBase):
    ...
    def calc_aper_correction(self):
        self.sigma_ap = kfuncs.sigma_aper_dispersion_misalign(aperModel1DDisp = self,
                        re_arcsec=self.galaxy.re_arcsec_circ,
                        re_mass_arcsec=self.galaxy.re_mass_arcsec_circ)
...
class AperModel1DRot(AperModel1DBase):
    ...
    def setup_calcs(self):
        ...
        # x_arr, y_arr are in *ARCSECONDS*
        xc = np.average(self.x_arr)
        yc = np.average(self.y_arr)
        x_arr_trim = self.x_arr[np.abs(self.x_arr-xc) <= self.instrument.PSF.PSF_FWHM]
        y_arr_trim = self.y_arr[np.abs(self.y_arr-yc) <= self.instrument.PSF.PSF_FWHM]

        # x_arr_flat = np.tile(x_arr_trim, (len(y_arr_trim),1))
        # y_arr_flat = np.tile(np.array([y_arr_trim]).T, (1,len(x_arr_trim)))
    ...
    def calc_aper_correction(self, recalc=False):
        ...
        self.V_RMS_aper = np.sqrt(V_aper**2 + sigma_at_aper**2)
    ...
    def update_kinProfile(self):
        ...
        if self.theta[0] is not None:
            theta_kinprof = np.array([self.theta[1], self.theta[2], None])
        else:
            theta_kinprof = np.array([self.theta[1], self.theta[2], 0.])
        ...
        if self.theta[0] is not None:
            if self.theta[0] != 0.:
                V_aper = self.kinProfile.vel(self.disp_aper_radius_arcsec, 0.)
                theta_kinprof = np.array([self.theta[1], self.theta[2], V_aper/self.theta[0]])
                self.kinProfile.update_theta(theta_kinprof)
            else:
                # v/sig = 0 == eg velocity = 0.
                theta_kinprof = np.array([0., self.theta[2], self.theta[1]])
                self.kinProfile.update_theta(theta_kinprof)
    ...
    def setup_model_grid(self):
        ...
        # Setup aperture width if not set:
        if self.extraction_width is None:
            re_arcsec_conv = 0.5*np.sqrt((2.*self.galaxy.re_arcsec)**2 + self.instrument.PSF.PSF_FWHM**2)
            minor_conv_arcsec = 0.5*np.sqrt((2.*self.galaxy.q*self.galaxy.re_arcsec)**2 + \
                        self.instrument.PSF.PSF_FWHM**2)
            q_conv = minor_conv_arcsec/re_arcsec_conv

            x_proj_conv = utils.x_proj_major(re_arcsec_conv, self.galaxy.delt_PA, q_conv)
            y_proj_conv = utils.y_proj_major(re_arcsec_conv, self.galaxy.delt_PA)
            # HWHM
        ...
        #########
        # Setup grid, values:
        self.sini = kfuncs.sin_i(self.galaxy.q, q0=self.galaxy.q0)
        self.i_rad = np.arcsin(self.sini)
        ...
        # coordinates at each point:
        self.x_arr = np.linspace(-(self.n_pix_x/2.+self.padX-0.5)*self.delt_x,
                        (self.n_pix_x/2.+self.padX-0.5)*self.delt_x, num=self.nX)

        self.z_arr = np.linspace(-(self.n_pix_z/2.-0.5)*self.delt_z,
                            (self.n_pix_z/2.-0.5)*self.delt_z, num=self.nZ)

        self.y_arr = np.linspace(-(self.n_pix_y/2.+self.padY-0.5)*self.delt_y,
        ...
        self.x_arr_full = np.tile(self.x_arr, (self.nZ, self.nY,1))
        self.y_arr_full = np.tile(np.array([self.y_arr]).T, (self.nZ, 1,self.nX))
        self.z_arr_full = np.repeat(self.z_arr,
                            self.nY*self.nX).reshape((self.nZ,self.nY,self.nX))

        self.xint, self.yint, self.zint = kfuncs.transform_int_3d(self.x_arr_full,
                                self.y_arr_full, self.z_arr_full,
                                self.galaxy.delt_PA, self.i_rad)
        self.r = np.sqrt(self.xint**2+self.yint**2)
    ...
    def calculate_kin_cube(self):
        ...
        if self.do_position_wave_shift or self.do_inst_res_conv_effective:
            if (self.delta_position_velspace is None) or (self.inst_disp_wave is None):
                ...
                self.delta_position_velspace, inst_disp_wave = \
                        kfuncs.calculate_effective_inst_res_lam_shift(I_wide_3d=I_wide,
                            xarr = self.x_arr,
                            yarr = self.y_arr,
        ...
        # shape: (nY,nX)
        self.sigsq_I_coll = np.sum(sigsq_I_wide, axis=0)*self.delt_z
        self.I_coll = np.sum(self.I_wide, axis=0)*self.delt_z
    ...
    def convolve_kin_arr_PSF(self):
        ...
        ###
        # kfuncs.PSF_convolve(spectra_cube, nWave, x_arr_flat, y_arr_flat, PSF_FWHM):
        #     # Convolve with seeing:
        #     # Should have shape (nWave, nY, nX)

        self.sigsq_I_conv_wide = kfuncs.PSF_convolve_flat(self.sigsq_I_coll, self.instrument.PSF)
        self.I_conv_wide =  kfuncs.PSF_convolve_flat(self.I_coll, self.instrument.PSF)
    ...
    def trim_sum_aperture(self):
        ...
        if self.extraction_method == 'optimal':
            ...
            self.G_y = np.tile(np.array([self.g_y]).transpose(), (1, self.I_conv.shape[1]))
        elif self.extraction_method == 'boxcar':
            self.G_y = np.ones(self.I_conv.shape)
        else:
            raise ValueError("Specified extraction method is not supported yet!: "+self.extraction_method)


        self.sigmasq_aper = np.sum(self.sigsq_I_conv*self.G_y*self.delt_x*self.delt_y)/\
                                np.sum(self.I_conv*self.G_y*self.delt_x*self.delt_y)
        self.sigma_aper = np.sqrt(self.sigmasq_aper)
...
class AperModel2D(object):
    ...
    def setup_calcs(self):
        ...
        conv_stamp = self.instrument.PSF.generate_conv_stamp(x_arr_trim, y_arr_trim)
        # Test new:
        kern3D = np.zeros(shape=(1, conv_stamp.shape[0], conv_stamp.shape[1],))
        kern3D[0, :, :] = conv_stamp
    ...
    def setup_model_grid(self):
        ...
        self.y_arr = np.linspace(-(self.padY-0.5)*self.delt_y,
        ...
        self.delt_wave = np.average(self.wave_arr[1:]-self.wave_arr[:-1])
    ...
    def check_do_upsample_wave(self):
        if (np.abs(self.kinProfile.dispProfile.theta) < self.sigma_upsample_value) :
            # Create new array that has finer sampling:
            self.delt_wave = self.delt_wave_real / np.float(self.adaptive_upsample_factor)
    ...
    def calculate_spectral_cube(self):
        ...
        #
        if self.absvalsigma:
            self.kinProfile.dispProfile.theta = np.abs(self.kinProfile.dispProfile.theta)
        if self.sigma_floor:
            if np.abs(self.kinProfile.dispProfile.theta) < self.sigma_floor_value:
                self.kinProfile.dispProfile.theta = np.array([self.sigma_floor_value])
        ...
        # start = time.time()
        self.spectra_cube = kfuncs.add_sigma_collapse_z(I_wide, V_wide, sigma_wide,
    ...
    def convolve_spectral_cube_PSF(self):
        ## Convolve the spectra cube with the PSF:
        self.spectra_cube_conv = kfuncs.PSF_convolve(self.spectra_cube, self.instrument.PSF)
    ...
    def trim_downsample_spectral_cube(self):
        ...
        self.model_out = np.sum(self.spectra_cube_conv,axis=2)*self.delt_x

        # Downsample to same size as emis_t: collapse the subpixels in the y direction
        if self.rebin_output:
            self.model_out = kfuncs.rebin(self.model_out,
    ...
    def convolve_instrument_resolution(self):
        ...
        if self.do_position_wave_shift:
            ...
            if self.inst_disp_wave.min() > 0.:
                self.model_out = kfuncs.inst_convol_complete(self.model_out,
