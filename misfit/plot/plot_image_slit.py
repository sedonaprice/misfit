...
def plot_image_slit(galaxy=None, instrument=None,
    ...
    if convolve & (conv_sigma is None):
        ...
        conv_FWHM = np.sqrt(instrument.PSF.PSF_FWHM**2 - instrument_img.PSF.PSF_FWHM**2)
    ...
    x0WCS_sci, y0WCS_sci = w.wcs_world2pix(galaxy.RA, galaxy.DEC, originWCS)
    x0WCS_sci -= 1. # Convert to np pixel coords
    y0WCS_sci -= 1. # Convert to np pixel coords
    if plot_galfit:
        x0_galfit, y0_galfit = w.wcs_world2pix(galaxy.sersicRA, galaxy.sersicDEC, originWCS)
    ...
    if rotate is True:
        ...
        x0_gal, y0_gal = rot_object_center(x0_galfit, y0_galfit, cent_pix,
                                -1.*slit_PA*d2r)
        x0_sci, y0_sci = rot_object_center(x0WCS_sci, y0WCS_sci,
                            cent_pix, -1.*slit_PA*d2r)
    else:
        x0_sci = x0WCS_sci.copy()
        y0_sci = y0WCS_sci.copy()
    ...
    if downsample is True:
        ...
        yarr_orig = np.linspace(0,np.shape(pstamp_trim)[0],
                            num=np.shape(pstamp_trim)[0], endpoint=False)
        xarr_orig = np.linspace(0,np.shape(pstamp_trim)[1],
                            num=np.shape(pstamp_trim)[1], endpoint=False)
        ...
        n_round = np.int(np.floor(np.shape(pstamp_trim)[0]*instrument_img.pixscale/instrument.pixscale))

        ### New:
        yarr_new = np.linspace(0, n_round, num=n_round, endpoint=False)
        # Center on m0:
        ycent = m0
        ...
        xarr_new = np.linspace(0, n_round, num=n_round, endpoint=False)
        # Center on center:
        xcent = (n_round-1)/2.
        xarr_new -= xcent
        ...
        # x is column, y is row: arrays are row, column
        cent_pix= np.array([(np.shape(pstamp_trim)[1]-1)/2.,
                (np.shape(pstamp_trim)[0]-1)/2.])
    ...
    trim_size_orig = np.repeat(2./pscale, 2)

    if n_spatial_pixels is not None:
        if np.average(trim_size_orig*pscale) != n_spatial_pixels*instrument.pixscale:
            trim_size = np.repeat(n_spatial_pixels*instrument.pixscale/pscale, 2)
        else:
    ...
    xlim = np.array([x0_gal-(trim_size[1])/2., x0_gal+(trim_size[1])/2.])
    ylim = np.array([y0_gal-(trim_size[0])/2., y0_gal+(trim_size[0])/2.])
    ...
    xlim_round = np.array([np.int(np.round(xlim[0])), np.int(np.round(xlim[1]))])
    ylim_round = np.array([np.int(np.round(ylim[0])), np.int(np.round(ylim[1]))])

    tmp_trim = pstamp_trim[ylim_round[0]:ylim_round[1],xlim_round[0]:xlim_round[1]].copy()
    tmp_trim = tmp_trim.flatten()
    range_spec = tmp_trim[np.isfinite(tmp_trim)].copy()
    range_spec = np.abs(range_spec)
    range_spec.sort()
    # Only consider the lowest 50\% *abs* points:
    range_spec = range_spec[0:np.int(np.ceil(len(range_spec)/2.))]
    median_err = np.median(range_spec)

    ext = 3.
    vmin = np.log10((-1.+ext)*median_err)
    vmax = np.log10((100.+ext)*median_err)


    ax.imshow(np.log10(pstamp_trim + median_err*(ext) ), cmap=cmap,
    ...
    try:
        ...
        slit_len = np.shape(pstamp_trim)[0]+20.
    ...
    y_shear = slit_wid/pscale*np.tan(off_angle*d2r)

    pos_11 = np.array([x0_sci-0.5*slit_wid/pscale, y0_sci-y0_off-0.5*slit_len/pscale])
    pos_12 = np.array([x0_sci-0.5*slit_wid/pscale, y0_sci-y0_off+0.5*slit_len/pscale])
    pos_21 = np.array([x0_sci+0.5*slit_wid/pscale, y0_sci-y0_off-0.5*slit_len/pscale])
    pos_22 = np.array([x0_sci+0.5*slit_wid/pscale, y0_sci-y0_off+0.5*slit_len/pscale])
    ...
    if rotate is True:
        if plot_galfit:
            ...
            galfit_angle = np.arctan(np.tan(galfit_angle*np.pi/180.))*180./np.pi
    else:
        if plot_galfit:
    ...
    if plot_galfit:
        ...
        if convolve:
            ...
            sigma_a = semi_major/(np.sqrt(2.*np.log(2.)))

            # print "FIX THIS!"
            # raise ValueError

            conv_sigma_galfit = instrument.PSF.PSF_FWHM/(2.*np.sqrt(2.*np.log(2.)))
            sigma_a_new = np.sqrt(conv_sigma_galfit**2 + sigma_a**2)
            semi_major = sigma_a_new*(np.sqrt(2.*np.log(2.)))

            # Do the same for semi-minor axis -- will get more smearing of one than
            #       the other, if the resolution element is ~= semi-minor axis
            sigma_b = semi_minor/(np.sqrt(2.*np.log(2.)))
            sigma_b_new = np.sqrt(conv_sigma_galfit**2 + sigma_b**2)
            semi_minor = sigma_b_new*(np.sqrt(2.*np.log(2.)))
        ...
        if show_galfit:
            ...
            # Semi-minor axis
            pos_11 = np.array([x0_gal-semi_minor/pscale, y0_gal])
            pos_22 = np.array([x0_gal+semi_minor/pscale, y0_gal])
            ...
            # Semi-major axis
            pos_11 = np.array([x0_gal, y0_gal-semi_major/pscale])
            pos_22 = np.array([x0_gal, y0_gal+semi_major/pscale])
...
def box_integrate(x, y, x_new, debug=False):
    ...
    delt_x_new = np.average(x_new[1:]-x_new[:-1])
    delt_x = np.average(x[1:]-x[:-1])
    ...
    for i in six.moves.xrange(len(x_new)):
        ...
        try:
            ...
            c_l = np.where(x_low >= low)[0]
            c_r = np.where(x_high <= high)[0]
            ind_c = np.array(list(set(c_l) & set(c_r)))
            ind_c.sort()

            int_y += np.sum(y[ind_c])*delt_x
        except:
            ind_c = 'None'
        ...
        try:
            ...
            ind_l = np.where(x_low <= low)[0].max()
        ...
        try:
            ...
            ind_r = np.where(x_high >= high)[0].min()
            # range corresponding to this interval:
            # (high - x_low[ind_r])
