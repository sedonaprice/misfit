...
def plot_param_corner(fitEmis2D, fileout=None,verbose=False,showLinked=True,
    ...
    if (fitEmis2D.theta_linked_posteriors is not None) & (showLinked):
        for j in range(len(fitEmis2D.theta_linked_posteriors)):
            ...
            col2arr = np.linspace(ylim[0], ylim[1],num=101)
            ...
            for i in range(len(fitEmis2D.kinModel.kinProfile.theta)):
                if i in fitEmis2D.theta_linked_posteriors[j]:
                    if i == fitEmis2D.theta_linked_posteriors[j][0]:
                        ...
                        # The fixed dimension:
                        theta_tmp = np.append(theta_tmp,
                                    np.array([col2arr]), axis=0)
                else:
                    if i > 0:
                        theta_tmp = np.append(theta_tmp,
                                    np.array([np.repeat(fitEmis2D.kinModel.bestfit.theta[i],
                                            len_tmp)]), axis=0)
                    else:
                        theta_tmp = np.array([np.repeat(fitEmis2D.kinModel.bestfit.theta[i],
                                    len_tmp)])
    ...
    # Annotate sigma
    wh_sig = np.where(np.array(fitEmis2D.mcmc_results.theta_names) == 'sigma0')[0][0]
    ...
    # Annotate V(R_E)
    wh_vre = np.where(np.array(fitEmis2D.mcmc_results.theta_names) == 'V_RE')[0][0]
