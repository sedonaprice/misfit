...
def plot_trace(sampler_dict, fitEmis2D, fileout=None):
    ...
    alpha = max(0.01, 1./_np.float(nWalkers))
    ...
    alphaTrace = 0.8
    lwTrace = 1.5
    trace_inds = _np.random.randint(0,nWalkers, size=nTraceWalkers)
    trace_colors = []
    for i in _six.moves.xrange(nTraceWalkers):
        trace_colors.append(cmap(1./_np.float(nTraceWalkers)*i))
    
    norm_inds = _np.setdiff1d(range(nWalkers), trace_inds)
    
    
    
    for k in _six.moves.xrange(nRows):
        axes.append(plt.subplot(gs[k,0]))
        
        axes[k].plot(sampler_dict['chain'][norm_inds,:,k].T, '-', color='black', alpha=alpha)
        
        for j in _six.moves.xrange(nTraceWalkers):
